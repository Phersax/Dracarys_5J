...
/*
 ...
 __HAL_TIM_SET_AUTORELOAD(htim, 20000);
 __HAL_TIM_SET_PRESCALER(&htim4, 20000);
...
/*count+=5;
 ...
 htim4.Instance->CR1 &= ~(TIM_CR1_CEN); // Disabilita il timer
 __HAL_TIM_SET_AUTORELOAD(htim, 40000);
 htim4.Instance->PSC = 40000;
 __HAL_TIM_SET_COMPARE(htim, TIM_CHANNEL_1,
 __HAL_TIM_GET_AUTORELOAD(htim)/2);

 htim4.Instance->EGR |= TIM_EGR_UG; // Genera un evento di aggiornamento
...
void HAL_TIM_PWM_PulseFinishedCallback(TIM_HandleTypeDef *htim) {
	...
	if (htim->Instance == TIM1) {
		...
		if (arr_des < (arr_current - ACCEL_RATE)
				&& cnt <= (int) (n_steps * 1 / 2)) { //acceleration
